...
G=load('grid4c_t2.mat'); tSample=G.tSample; theta0=G.theta0; task=G.task; Nobs=200; nSubj=30;
sz=[numel(G.su_range) numel(G.ss_range) numel(G.d_range) numel(G.r_range)];
Sigma=statsCovariance2(theta0,Nobs,400,60000,task,tSample);
thr=hotelling(size(G.Spred,1),400);
P3=reshape(G.Spred,size(G.Spred,1),[]);
...
for k=1:nSubj
  s=ofcSummary2(theta0,Nobs,70000+k,task,tSample);
  ee=P3-s; D=reshape(sum(ee.*(Sigma\ee),1),sz);
